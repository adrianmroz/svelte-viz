...
<SvgResponsiveStage>
	<Gutters as={GroupStage} top={margin} bottom={margin} left={margin} right={margin}>
		<svelte:fragment slot="center">
			<RowLayout {data} {rowDomain} getRowValue={name} {rowScale} let:scene={rowScene} let:datum>
				<GroupStage scene={rowScene}>
					<text
						alignment-baseline="middle"
						y={rowScene.height / 2}
						font-size={rowScene.height * 0.7}
					>
						{datum.name}
					</text>
					<Inset as={GroupStage} left={labelLength} top={-(rowScene.height * 0.8)}>
						<XYFrame
							data={[datum]}
							{getX}
							{xDomain}
